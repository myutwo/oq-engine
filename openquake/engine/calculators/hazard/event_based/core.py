...
def compute_gmfs_and_curves(job_id, ses_ruptures, sitecol):
    ...
    result = {}  # trt_model_id -> (curves_by_gsim, calc)
    ...
    calc.rlzs_by_gsim = rlzs_by_gsim
    ...
    if getattr(hc, 'hazard_curves_from_gmfs', None):
        with EnginePerformanceMonitor(
            ...
            result[trt_model.id] = (calc.to_haz_curves(
                ...
                hc.investigation_time, hc.ses_per_logic_tree_path), calc)
    else:
        result[trt_model.id] = ([], calc)
...
class GmfCalculator(object):
    ...
    def __init__(self, sorted_imts, sorted_gsims, trt_model_id,
        ...
        self.rg_per_site = AccumDict()  # gsim, imt_str,site_id -> [(rup, gmv)]
        self.rlzs_by_gsim = {}  # set in the task
    ...
    def calc_gmfs(self, r_sites, rupture, rupid_seed_pairs):
        ...
        .gmvs_per_site and .rups_per_site.
            ...
            for rupid, seed in rupid_seed_pairs:
                for imt_str, gmvs in computer.compute(seed):
                    for site_id, gmv in zip(r_sites.sids, gmvs):
                        self.rg_per_site += {
                            (gsim_name, imt_str, site_id): [(rupid, gmv)]}
    ...
    def to_haz_curves(self, sids, imtls, invest_time, num_ses):
        ...
        for (gsim, imt, site_id), rgs in self.rg_per_site.iteritems():
            rups, gmvs = zip(*rgs)
...
def save_gmfs(job_id, trt_model_id, rg_per_site):
    ...
    """
    rlzs_by_gsim = models.TrtModel.objects.get(
        pk=trt_model_id).get_rlzs_by_gsim()
    nrows = 0
    for (gsim_name, imt_str, site_id), rgs in rg_per_site.iteritems():
        rups, gmvs = zip(*rgs)
...
class EventBasedHazardCalculator(general.BaseHazardCalculator):
    ...
    def generate_gmfs_and_curves(self):
        ...
        return tasks.apply_reduce(
            ...
            self.agg_gmfs, key=lambda sr: sr.trt_id)

    @EnginePerformanceMonitor.monitor
    def agg_gmfs(self, acc, result):
        """
        """
        for trt_model_id, (curves_by_gsim, calc) in result.iteritems():
            acc += {trt_model_id: calc.rg_per_site}

            # as a side effect accumulate the hazard curves, if any
            for gsim, probs in curves_by_gsim:
                pnes = []
                for prob, zero in itertools.izip(probs, self.zeros):
                    pnes.append(1 - (zero if general.all_equal(prob, 0)
                                     else prob))
                pnes1 = numpy.array(pnes)
                pnes2 = 1 - self.acc.get((trt_model_id, gsim), self.zeros)
                self.acc[trt_model_id, gsim] = 1 - pnes1 * pnes2
